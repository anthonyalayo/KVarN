...
class KVQuantMode(IntEnum):
    ...
    # KVarN variance-normalized tile quant. Member names mirror the
    # cache-dtype strings; the packed layout is published by the KVarN
    # backend's customize_spec (state_content_bytes), not by the mode tag.
    KVARN_K4V2_G128 = 10
    KVARN_K4V4_G128 = 11
    KVARN_K4V2_G64 = 12
    KVARN_K4V4_G64 = 13
    KVARN_MLA_K4_G128 = 14
...
def get_kv_quant_mode(kv_cache_dtype: str) -> KVQuantMode:
    ...
    if isinstance(kv_cache_dtype, str) and kv_cache_dtype.startswith("kvarn_"):
        # KVarN reuses vLLM's spec plumbing but publishes its own packed
        # per-(token, head) layout via KVarNAttentionBackend.customize_spec.
        # Keeping the mode non-NONE stops skip-layer dtype resolution from
        # treating kvarn layers as unquantized.
        return KVQuantMode[kv_cache_dtype.upper()]
