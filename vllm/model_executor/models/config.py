...
class Gemma4Config(VerifyAndUpdateConfig):
    ...
    def verify_and_update_config(vllm_config: "VllmConfig") -> None:
        ...
        # KVarN runs ALL layers through its own backend (per-layer head_size, up to
        # 512), so there is no mixed-backend divergence and no standard backend
        # (incl. TRITON_ATTN, which cannot serve kvarn cache dtypes) should be
        # forced for dense kvarn_ dtypes.
